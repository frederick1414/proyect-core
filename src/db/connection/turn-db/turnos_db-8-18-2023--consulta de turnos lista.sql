-- phpMyAdmin SQL Dump
-- version 5.2.1
-- https://www.phpmyadmin.net/
--
-- Host: 127.0.0.1
-- Generation Time: Aug 19, 2023 at 12:47 AM
-- Server version: 10.4.28-MariaDB
-- PHP Version: 8.0.28

SET SQL_MODE = "NO_AUTO_VALUE_ON_ZERO";
START TRANSACTION;
SET time_zone = "+00:00";


/*!40101 SET @OLD_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT */;
/*!40101 SET @OLD_CHARACTER_SET_RESULTS=@@CHARACTER_SET_RESULTS */;
/*!40101 SET @OLD_COLLATION_CONNECTION=@@COLLATION_CONNECTION */;
/*!40101 SET NAMES utf8mb4 */;

--
-- Database: `turnos_db`
--

-- --------------------------------------------------------

--
-- Table structure for table `brand`
--

CREATE TABLE `brand` (
  `BRAND_ID` varchar(255) NOT NULL,
  `DESCRIPTION` varchar(255) DEFAULT NULL,
  `STATUS` char(1) DEFAULT 'A',
  `BUSINESS_ID` varchar(255) NOT NULL,
  `CREATED_DATE` datetime DEFAULT NULL,
  `CREATED_USER` varchar(100) DEFAULT NULL,
  `UPDATED_DATE` datetime DEFAULT NULL,
  `UPDATED_USER` varchar(100) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;

-- --------------------------------------------------------

--
-- Table structure for table `business`
--

CREATE TABLE `business` (
  `BUSINESS_ID` varchar(255) NOT NULL,
  `BUSINESS_NAME` varchar(255) NOT NULL,
  `ACRONYM` varchar(100) DEFAULT NULL,
  `ADDRESS` varchar(255) DEFAULT NULL,
  `PHONE_1` varchar(50) DEFAULT NULL,
  `PHONE_2` varchar(50) DEFAULT NULL,
  `EMAIL_1` varchar(255) DEFAULT NULL,
  `EMAIL_2` varchar(255) DEFAULT NULL,
  `GENERIC_FIELD_1` varchar(255) DEFAULT NULL,
  `GENERIC_FIELD_2` varchar(255) DEFAULT NULL,
  `SLOGAN` varchar(255) DEFAULT NULL,
  `LOGO` varchar(500) DEFAULT NULL,
  `STATUS` char(1) NOT NULL,
  `CREATED_DATE` datetime DEFAULT NULL,
  `CREATED_USER` varchar(100) DEFAULT NULL,
  `UPDATED_DATE` datetime DEFAULT NULL,
  `UPDATED_USER` varchar(100) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

--
-- Dumping data for table `business`
--

INSERT INTO `business` (`BUSINESS_ID`, `BUSINESS_NAME`, `ACRONYM`, `ADDRESS`, `PHONE_1`, `PHONE_2`, `EMAIL_1`, `EMAIL_2`, `GENERIC_FIELD_1`, `GENERIC_FIELD_2`, `SLOGAN`, `LOGO`, `STATUS`, `CREATED_DATE`, `CREATED_USER`, `UPDATED_DATE`, `UPDATED_USER`) VALUES
('001', 'TEST', 'TST', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, 'A', NULL, NULL, NULL, NULL);

-- --------------------------------------------------------

--
-- Table structure for table `categories`
--

CREATE TABLE `categories` (
  `CATEGORY_ID` varchar(255) NOT NULL,
  `CATEGORY_NAME` varchar(100) NOT NULL,
  `DESCRIPTION` varchar(255) DEFAULT NULL,
  `STATUS` char(1) NOT NULL,
  `CREATED_DATE` datetime DEFAULT NULL,
  `CREATED_USER` varchar(100) DEFAULT NULL,
  `UPDATED_DATE` datetime DEFAULT NULL,
  `UPDATED_USER` varchar(100) DEFAULT NULL,
  `BUSINESS_ID` varchar(255) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

--
-- Dumping data for table `categories`
--

INSERT INTO `categories` (`CATEGORY_ID`, `CATEGORY_NAME`, `DESCRIPTION`, `STATUS`, `CREATED_DATE`, `CREATED_USER`, `UPDATED_DATE`, `UPDATED_USER`, `BUSINESS_ID`) VALUES
('1', 'ALIMENTOS', 'ALIMENTOS', 'A', NULL, NULL, NULL, NULL, '001');

-- --------------------------------------------------------

--
-- Table structure for table `contact`
--

CREATE TABLE `contact` (
  `CONTACT_ID` varchar(255) NOT NULL,
  `CUSTOMER_ID` varchar(255) DEFAULT NULL,
  `VALUE` varchar(255) DEFAULT NULL,
  `CONTACT_TYPE` char(1) DEFAULT NULL,
  `MAIN` int(11) DEFAULT NULL,
  `STATUS` char(1) DEFAULT NULL,
  `BUSINESS_ID` varchar(255) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

-- --------------------------------------------------------

--
-- Table structure for table `country`
--

CREATE TABLE `country` (
  `COUNTRY_ID` varchar(100) NOT NULL,
  `DESCRIPTION` varchar(255) DEFAULT NULL,
  `STATUS` char(1) DEFAULT 'A',
  `CREATED_DATE` datetime DEFAULT NULL,
  `CREATED_USER` varchar(100) DEFAULT NULL,
  `UPDATED_DATE` datetime DEFAULT NULL,
  `UPDATED_USER` varchar(100) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;

--
-- Dumping data for table `country`
--

INSERT INTO `country` (`COUNTRY_ID`, `DESCRIPTION`, `STATUS`, `CREATED_DATE`, `CREATED_USER`, `UPDATED_DATE`, `UPDATED_USER`) VALUES
('161c9cdb-18a0-471c-990d-028d4a453ff7', 'HAITI', 'A', '2023-04-27 19:58:13', 'ADMIN', '2023-04-27 19:58:48', 'ADMIN'),
('e3a0cb56-627d-4684-bfe7-07b220998cb9', 'REPÚBLICA DOMINICANA', 'A', '2023-04-27 19:55:56', 'ADMIN', '2023-04-27 19:57:52', 'ADMIN');

-- --------------------------------------------------------

--
-- Table structure for table `customers`
--

CREATE TABLE `customers` (
  `CUSTOMER_ID` varchar(255) NOT NULL,
  `LAST_NAME` varchar(255) NOT NULL,
  `FIRST_NAME` varchar(255) NOT NULL,
  `BIRTHDATE` date DEFAULT NULL,
  `ADDRESS` varchar(255) DEFAULT NULL,
  `PHONE_NUMBER` varchar(50) DEFAULT NULL,
  `EMAIL` varchar(255) DEFAULT NULL,
  `STATUS` char(1) NOT NULL,
  `CREATED_DATE` datetime DEFAULT NULL,
  `CREATED_USER` varchar(100) DEFAULT NULL,
  `UPDATED_DATE` datetime DEFAULT NULL,
  `UPDATED_USER` varchar(100) DEFAULT NULL,
  `BUSINESS_ID` varchar(255) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

--
-- Dumping data for table `customers`
--

INSERT INTO `customers` (`CUSTOMER_ID`, `LAST_NAME`, `FIRST_NAME`, `BIRTHDATE`, `ADDRESS`, `PHONE_NUMBER`, `EMAIL`, `STATUS`, `CREATED_DATE`, `CREATED_USER`, `UPDATED_DATE`, `UPDATED_USER`, `BUSINESS_ID`) VALUES
('1', 'Delgado', 'Pablo', '0000-00-00', 'la vega', '809-555-6663', 'pdelgado@correo.com', 'A', '0000-00-00 00:00:00', 'system', NULL, NULL, '001');

-- --------------------------------------------------------

--
-- Table structure for table `employees`
--

CREATE TABLE `employees` (
  `EMPLOYEE_ID` varchar(255) NOT NULL,
  `LAST_NAME` varchar(255) NOT NULL,
  `FIRST_NAME` varchar(255) NOT NULL,
  `BIRTHDATE` datetime DEFAULT NULL,
  `HIREDATE` datetime DEFAULT NULL,
  `ADDRESS` varchar(255) DEFAULT NULL,
  `CITY` varchar(100) DEFAULT NULL,
  `PHONE_NUMBER` varchar(50) DEFAULT NULL,
  `EMAIL` varchar(255) DEFAULT NULL,
  `POSITION_ID` varchar(255) DEFAULT NULL,
  `BUSINESS_ID` varchar(255) DEFAULT NULL,
  `STATUS` char(1) NOT NULL,
  `CREATED_DATE` datetime DEFAULT NULL,
  `CREATED_USER` varchar(100) DEFAULT NULL,
  `UPDATED_DATE` datetime DEFAULT NULL,
  `UPDATED_USER` varchar(100) DEFAULT NULL,
  `WAITING_TIME` int(11) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

--
-- Dumping data for table `employees`
--

INSERT INTO `employees` (`EMPLOYEE_ID`, `LAST_NAME`, `FIRST_NAME`, `BIRTHDATE`, `HIREDATE`, `ADDRESS`, `CITY`, `PHONE_NUMBER`, `EMAIL`, `POSITION_ID`, `BUSINESS_ID`, `STATUS`, `CREATED_DATE`, `CREATED_USER`, `UPDATED_DATE`, `UPDATED_USER`, `WAITING_TIME`) VALUES
('1', 'JUAN', 'PEREZ', '0000-00-00 00:00:00', '0000-00-00 00:00:00', 'LA VEGA', 'LA VEGA', '8095663322', 'JPEREZ@GMAIL.COM', '1', '001', 'A', '0000-00-00 00:00:00', 'SYSTEM', NULL, NULL, 30);

-- --------------------------------------------------------

--
-- Table structure for table `menu`
--

CREATE TABLE `menu` (
  `ID` varchar(255) NOT NULL,
  `BUSINESS_ID` varchar(255) DEFAULT NULL,
  `NAME` varchar(100) DEFAULT NULL,
  `DESCRIPTION` varchar(255) DEFAULT NULL,
  `PARENT_MENU` varchar(255) DEFAULT NULL,
  `STATUS` char(1) DEFAULT NULL,
  `MODULE` varchar(100) DEFAULT NULL
) ENGINE=InnoDB AVG_ROW_LENGTH=630 DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;

--
-- Dumping data for table `menu`
--

INSERT INTO `menu` (`ID`, `BUSINESS_ID`, `NAME`, `DESCRIPTION`, `PARENT_MENU`, `STATUS`, `MODULE`) VALUES
('01', '001', 'Dashboard', 'Dashboard', '1', 'A', '/'),
('02', '001', 'Clientes', 'Clientes', '1', 'A', NULL),
('02-1', '001', 'Clientes', 'Clientes', '02', 'A', '/clients'),
('02-2', '001', 'Contactos', 'Contactos', '02', 'A', '/contacts'),
('03', '001', 'Inventario', 'Inventario', '1', 'A', NULL),
('03-1', '001', 'Productos', 'Productos', '03', 'A', '/products'),
('03-2', '001', 'Categorias', 'Categorias', '03', 'A', '/category'),
('04', '001', 'Ordenes', 'Ordenes', '1', 'A', NULL),
('04-1', '001', 'Nueva orden', 'Nueva orden', '04', 'A', '/new_order'),
('04-2', '001', 'Cuentas por cobrar', 'Cuentas por cobrar', '04', 'A', '/cxc'),
('04-3', '001', 'Historial de ventas', 'Historial de ventas', '04', 'A', '/orders'),
('05', '001', 'Compras', 'Compras', '1', 'A', NULL),
('05-1', '001', 'Nueva compra', 'Nueva compra', '05', 'A', '/new_purchase'),
('05-2', '001', 'Cuentas por pagar', 'Cuentas por pagar', '05', 'A', '/cxp'),
('05-3', '001', 'Historial de compras', 'historial de compras', '05', 'A', '/purchases'),
('06', '001', 'Proveedores', 'Proveedores', '1', 'A', '/suppliers'),
('07', '001', 'RRHH', 'RRHH', '1', 'A', NULL),
('07-1', '001', 'Empleados', 'Empleados', '07', 'A', '/employees'),
('07-2', '001', 'Posiciones', 'Posiciones', '07', 'A', '/positions'),
('08', '001', 'Administracion', 'Administracion', '1', 'A', ''),
('08-1', '001', 'Usuarios', 'Usuarios', '08', 'A', '/users'),
('08-2', '001', 'Roles', 'Roles', '08', 'A', '/roles'),
('08-3', '001', 'Empresa', 'Empresa', '08', 'A', '/business'),
('09', '001', 'Configuracion', 'Configuracion', '1', 'A', NULL),
('09-1', '001', 'Perfil', 'Perfil', '09', 'A', '/profile'),
('1', '001', 'Menu General', 'MENU GENERAL', NULL, 'A', NULL);

-- --------------------------------------------------------

--
-- Table structure for table `orders`
--

CREATE TABLE `orders` (
  `ORDER_ID` varchar(255) NOT NULL,
  `BUSINESS_ID` varchar(255) NOT NULL,
  `CUSTOMER_ID` varchar(255) DEFAULT NULL,
  `EMPLOYEE_ID` varchar(255) DEFAULT NULL,
  `ORDER_DATE` datetime DEFAULT NULL,
  `REQUIREDATE` datetime DEFAULT NULL,
  `AMOUNT_DESC` decimal(10,2) NOT NULL,
  `TAX_AMOUNT` decimal(10,2) NOT NULL,
  `TAXED_AMOUNT` decimal(10,2) NOT NULL,
  `EXEMPT_AMOUNT` decimal(10,2) NOT NULL,
  `NOTES` varchar(255) DEFAULT NULL,
  `AMOUNT` decimal(10,2) NOT NULL,
  `NET_AMOUNT` decimal(10,2) NOT NULL,
  `PERC_DISCOUNT` decimal(4,2) NOT NULL,
  `STATUS` char(1) NOT NULL,
  `CREATED_DATE` datetime DEFAULT NULL,
  `CREATED_USER` varchar(100) DEFAULT NULL,
  `UPDATED_DATE` datetime DEFAULT NULL,
  `UPDATED_USER` varchar(100) DEFAULT NULL
) ENGINE=InnoDB AVG_ROW_LENGTH=16384 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

--
-- Dumping data for table `orders`
--

INSERT INTO `orders` (`ORDER_ID`, `BUSINESS_ID`, `CUSTOMER_ID`, `EMPLOYEE_ID`, `ORDER_DATE`, `REQUIREDATE`, `AMOUNT_DESC`, `TAX_AMOUNT`, `TAXED_AMOUNT`, `EXEMPT_AMOUNT`, `NOTES`, `AMOUNT`, `NET_AMOUNT`, `PERC_DISCOUNT`, `STATUS`, `CREATED_DATE`, `CREATED_USER`, `UPDATED_DATE`, `UPDATED_USER`) VALUES
('1', '001', '1', '1', '0000-00-00 00:00:00', NULL, 0.00, 0.00, 0.00, 0.00, NULL, 0.00, 0.00, 0.00, 'A', '0000-00-00 00:00:00', 'SYSTEM', NULL, NULL);

-- --------------------------------------------------------

--
-- Table structure for table `order_details`
--

CREATE TABLE `order_details` (
  `ORDER_DETAIL_ID` varchar(255) NOT NULL,
  `ORDER_ID` varchar(255) NOT NULL,
  `PRODUCT_ID` varchar(255) NOT NULL,
  `BUSINESS_ID` varchar(255) NOT NULL,
  `UNIT_PRICE` decimal(12,2) DEFAULT NULL,
  `QUANTITY` decimal(12,2) DEFAULT NULL,
  `DISCOUNT` decimal(12,2) DEFAULT NULL,
  `STATUS` char(1) DEFAULT NULL,
  `CREATED_DATE` datetime DEFAULT NULL,
  `CREATED_USER` varchar(100) DEFAULT NULL,
  `UPDATED_DATE` datetime DEFAULT NULL,
  `UPDATED_USER` varchar(100) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;

--
-- Dumping data for table `order_details`
--

INSERT INTO `order_details` (`ORDER_DETAIL_ID`, `ORDER_ID`, `PRODUCT_ID`, `BUSINESS_ID`, `UNIT_PRICE`, `QUANTITY`, `DISCOUNT`, `STATUS`, `CREATED_DATE`, `CREATED_USER`, `UPDATED_DATE`, `UPDATED_USER`) VALUES
('1', '1', '1', '001', 100.00, 1.00, 1.00, 'A', '0000-00-00 00:00:00', 'system', NULL, NULL);

-- --------------------------------------------------------

--
-- Table structure for table `position`
--

CREATE TABLE `position` (
  `POSITION_ID` varchar(255) NOT NULL,
  `DESCRIPTION` varchar(255) DEFAULT NULL,
  `STATUS` char(1) NOT NULL,
  `CREATED_DATE` datetime DEFAULT NULL,
  `CREATED_USER` varchar(100) DEFAULT NULL,
  `UPDATED_DATE` datetime DEFAULT NULL,
  `UPDATED_USER` varchar(100) DEFAULT NULL,
  `BUSINESS_ID` varchar(255) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

--
-- Dumping data for table `position`
--

INSERT INTO `position` (`POSITION_ID`, `DESCRIPTION`, `STATUS`, `CREATED_DATE`, `CREATED_USER`, `UPDATED_DATE`, `UPDATED_USER`, `BUSINESS_ID`) VALUES
('1', 'BARBERO', 'A', '0000-00-00 00:00:00', 'System', NULL, NULL, '001');

-- --------------------------------------------------------

--
-- Table structure for table `products`
--

CREATE TABLE `products` (
  `PRODUCT_ID` varchar(255) NOT NULL,
  `PRODUCT_NAME` varchar(100) NOT NULL,
  `CATEGORY_ID` varchar(255) DEFAULT NULL,
  `QUANTITY_PER_UNIT` float(9,2) DEFAULT NULL,
  `UNIT_PRICE` float(9,2) DEFAULT NULL,
  `UNIT_IN_STOCK` int(11) DEFAULT NULL,
  `UNIT_IN_ORDER` int(11) DEFAULT NULL,
  `BARCODE` varchar(255) DEFAULT NULL,
  `DISCONTINUED` char(1) DEFAULT NULL,
  `STATUS` char(1) NOT NULL,
  `CREATED_DATE` datetime DEFAULT NULL,
  `CREATED_USER` varchar(100) DEFAULT NULL,
  `UPDATED_DATE` datetime DEFAULT NULL,
  `UPDATED_USER` varchar(100) DEFAULT NULL,
  `BUSINESS_ID` varchar(255) NOT NULL,
  `DESCRIPTION` varchar(255) DEFAULT NULL,
  `MODEL_NO` varchar(50) DEFAULT NULL,
  `MANUFACTORING_DATE` datetime DEFAULT NULL,
  `EXPIRATION_DATE` datetime DEFAULT NULL,
  `SERIAL_NUMBER` varchar(100) DEFAULT NULL,
  `INSTRUCTIONS` varchar(255) DEFAULT NULL,
  `SAFETY_WARNINGS` varchar(255) DEFAULT NULL,
  `GUARANTEE` varchar(255) DEFAULT NULL,
  `COUNTRY_ID` varchar(255) DEFAULT NULL,
  `BRAND_ID` varchar(255) DEFAULT NULL
) ENGINE=InnoDB AVG_ROW_LENGTH=16384 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

--
-- Dumping data for table `products`
--

INSERT INTO `products` (`PRODUCT_ID`, `PRODUCT_NAME`, `CATEGORY_ID`, `QUANTITY_PER_UNIT`, `UNIT_PRICE`, `UNIT_IN_STOCK`, `UNIT_IN_ORDER`, `BARCODE`, `DISCONTINUED`, `STATUS`, `CREATED_DATE`, `CREATED_USER`, `UPDATED_DATE`, `UPDATED_USER`, `BUSINESS_ID`, `DESCRIPTION`, `MODEL_NO`, `MANUFACTORING_DATE`, `EXPIRATION_DATE`, `SERIAL_NUMBER`, `INSTRUCTIONS`, `SAFETY_WARNINGS`, `GUARANTEE`, `COUNTRY_ID`, `BRAND_ID`) VALUES
('1', 'CARNE', '1', 10.00, 500.00, 50, 1, '000001010', '0', 'A', '0000-00-00 00:00:00', 'SYSTEM', NULL, NULL, '001', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL),
('19949782-5f13-4ca6-97cc-8463901013e0', 'CARNE', '1', 1.00, 500.00, 12, 12, '001', 'N', '', '2023-04-27 20:24:43', 'ADMIN', NULL, NULL, '001', 'CARNE FRSCA', NULL, NULL, NULL, NULL, NULL, NULL, 'GARANTIA DE PRODUCTO', '1', '1');

-- --------------------------------------------------------

--
-- Table structure for table `product_image`
--

CREATE TABLE `product_image` (
  `ID` varchar(255) NOT NULL,
  `PRODUCT_ID` varchar(255) NOT NULL,
  `URL` varchar(255) DEFAULT NULL,
  `NAME` varchar(255) DEFAULT NULL,
  `EXTENSION` varchar(45) DEFAULT NULL,
  `STATUS` char(1) DEFAULT 'A',
  `CREATED_DATE` datetime DEFAULT NULL,
  `CREATED_USER` varchar(100) DEFAULT NULL,
  `UPDATED_DATE` datetime DEFAULT NULL,
  `UPDATED_USER` varchar(100) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;

-- --------------------------------------------------------

--
-- Table structure for table `product_supplier`
--

CREATE TABLE `product_supplier` (
  `PRODUCT_ID` varchar(255) NOT NULL,
  `SUPPLIER_ID` varchar(255) DEFAULT NULL,
  `STATUS` char(1) NOT NULL,
  `CREATED_DATE` datetime DEFAULT NULL,
  `CREATED_USER` varchar(100) DEFAULT NULL,
  `UPDATED_DATE` datetime DEFAULT NULL,
  `UPDATED_USER` varchar(100) DEFAULT NULL,
  `BUSINESS_ID` varchar(255) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

-- --------------------------------------------------------

--
-- Table structure for table `rol`
--

CREATE TABLE `rol` (
  `ROL_ID` varchar(255) NOT NULL,
  `NAME` varchar(100) NOT NULL,
  `NOTES` varchar(255) DEFAULT NULL,
  `STATUS` char(1) NOT NULL,
  `CREATED_DATE` datetime DEFAULT NULL,
  `CREATED_USER` varchar(100) DEFAULT NULL,
  `UPDATED_DATE` datetime DEFAULT NULL,
  `UPDATED_USER` varchar(100) DEFAULT NULL,
  `BUSINESS_ID` varchar(255) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

--
-- Dumping data for table `rol`
--

INSERT INTO `rol` (`ROL_ID`, `NAME`, `NOTES`, `STATUS`, `CREATED_DATE`, `CREATED_USER`, `UPDATED_DATE`, `UPDATED_USER`, `BUSINESS_ID`) VALUES
('291ae129-dd64-47db-8519-a7dad4701048', 'HOST', 'Host rol', 'A', '2023-02-10 20:25:28', NULL, NULL, NULL, '001'),
('d73dab3f-2c21-4265-af1f-cc20df746a90', 'ADMIN', 'Admin rol', 'A', '2023-02-10 20:25:49', NULL, NULL, NULL, '001');

-- --------------------------------------------------------

--
-- Table structure for table `service`
--

CREATE TABLE `service` (
  `service_id` varchar(5) NOT NULL,
  `description` varchar(100) NOT NULL,
  `BUSINESS_ID` varchar(255) NOT NULL,
  `ESTATUS` char(5) NOT NULL,
  `CREATED_DATE` datetime NOT NULL,
  `UPDATE_DATE` datetime NOT NULL,
  `EMPLOYEE_ID` varchar(10) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

--
-- Dumping data for table `service`
--

INSERT INTO `service` (`service_id`, `description`, `BUSINESS_ID`, `ESTATUS`, `CREATED_DATE`, `UPDATE_DATE`, `EMPLOYEE_ID`) VALUES
('1', 'corte nomal', '001', 'A', '2023-08-13 22:50:09', '2023-08-13 22:50:09', '1');

-- --------------------------------------------------------

--
-- Table structure for table `suppliers`
--

CREATE TABLE `suppliers` (
  `SUPPLIER_ID` varchar(255) NOT NULL,
  `COMPANY_NAME` varchar(255) NOT NULL,
  `CONTACT_NAME` varchar(100) NOT NULL,
  `CONTACT_TITLE` varchar(100) DEFAULT NULL,
  `PHONE_NUMBER` varchar(50) DEFAULT NULL,
  `EMAIL` varchar(255) DEFAULT NULL,
  `ID_CARD` varchar(100) DEFAULT NULL,
  `CITY` varchar(100) DEFAULT NULL,
  `ADDRESS` varchar(255) DEFAULT NULL,
  `STATUS` char(1) NOT NULL,
  `CREATED_DATE` datetime DEFAULT NULL,
  `CREATED_USER` varchar(100) DEFAULT NULL,
  `UPDATED_DATE` datetime DEFAULT NULL,
  `UPDATED_USER` varchar(100) DEFAULT NULL,
  `BUSINESS_ID` varchar(255) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

-- --------------------------------------------------------

--
-- Table structure for table `turns`
--

CREATE TABLE `turns` (
  `TURN_ID` varchar(255) NOT NULL,
  `EMPLOYEE_ID` varchar(255) NOT NULL,
  `BUSINESS_ID` varchar(100) NOT NULL,
  `USER_ID` varchar(255) NOT NULL,
  `USERNAME` varchar(255) NOT NULL,
  `SERVICE_ID` varchar(255) NOT NULL,
  `PHONE` varchar(15) NOT NULL,
  `TIME` datetime NOT NULL,
  `ESTATUS` char(1) NOT NULL,
  `CREATE_DATE` datetime NOT NULL,
  `UPDATE_DATE` datetime NOT NULL,
  `CREATED_USER` varchar(255) NOT NULL,
  `UPDATED_USER` varchar(255) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

--
-- Dumping data for table `turns`
--

INSERT INTO `turns` (`TURN_ID`, `EMPLOYEE_ID`, `BUSINESS_ID`, `USER_ID`, `USERNAME`, `SERVICE_ID`, `PHONE`, `TIME`, `ESTATUS`, `CREATE_DATE`, `UPDATE_DATE`, `CREATED_USER`, `UPDATED_USER`) VALUES
('1', '1', '001', '413c80b1-400d-456f-999a-05f79302ce63', 'FREDERICK', '1', '8299827269', '2023-08-16 22:15:03', 'A', '2023-08-16 22:15:03', '2023-08-16 22:15:03', 'TEST', '');

-- --------------------------------------------------------

--
-- Table structure for table `user`
--

CREATE TABLE `user` (
  `USER_ID` varchar(255) NOT NULL,
  `USERNAME` varchar(100) NOT NULL,
  `PASSWORD` varchar(100) NOT NULL,
  `EMAIL` varchar(100) NOT NULL,
  `PHONE` varchar(15) NOT NULL,
  `IMAGE` varchar(500) DEFAULT NULL,
  `EMPLOYEE_ID` varchar(255) DEFAULT NULL,
  `STATUS` char(1) NOT NULL,
  `CREATED_DATE` datetime DEFAULT NULL,
  `CREATED_USER` varchar(100) DEFAULT NULL,
  `UPDATED_DATE` datetime DEFAULT NULL,
  `UPDATED_USER` varchar(100) DEFAULT NULL,
  `ROL_ID` varchar(255) DEFAULT NULL,
  `BUSINESS_ID` varchar(255) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

--
-- Dumping data for table `user`
--

INSERT INTO `user` (`USER_ID`, `USERNAME`, `PASSWORD`, `EMAIL`, `PHONE`, `IMAGE`, `EMPLOYEE_ID`, `STATUS`, `CREATED_DATE`, `CREATED_USER`, `UPDATED_DATE`, `UPDATED_USER`, `ROL_ID`, `BUSINESS_ID`) VALUES
('165d3ba4-0b34-45b6-9cc6-ff9d41471e67', 'SECRE1', '$2a$15$5TwuRv56P85T0uC7Hhk/eu1jjiWDO4rVZTiKkXj8eoLKTfI9gVyHC', 'MARY_JANE@CORREO.COM', '', NULL, NULL, 'A', '2023-02-10 20:29:49', 'TEST', NULL, NULL, '291ae129-dd64-47db-8519-a7dad4701048', '001'),
('352dab61-b4af-4ff6-89b1-b1a94eb7c380', 'ADMIN', '$2a$15$R3aTUZiywgW3I883HUrteevLehUxJ4bikj2xjqhV1bbZbPuk9Q0i.', 'PETER_PARKER_1@CORREO.COM', '', NULL, NULL, 'A', '2023-02-10 20:29:00', 'TEST', NULL, NULL, 'd73dab3f-2c21-4265-af1f-cc20df746a90', '001'),
('413c80b1-400d-456f-999a-05f79302ce63', 'FREDERICK', '$2a$15$R3aTUZiywgW3I883HUrteevLehUxJ4bikj2xjqhV1bbZbPuk9Q0i.', 'frederick@gamil.com', '8299827269', NULL, NULL, 'A', '2023-08-13 16:11:33', 'ADMIN', NULL, NULL, 'd73dab3f-2c21-4265-af1f-cc20df746a90', '001');

--
-- Indexes for dumped tables
--

--
-- Indexes for table `brand`
--
ALTER TABLE `brand`
  ADD PRIMARY KEY (`BRAND_ID`),
  ADD KEY `fkBusinessBrand_idx` (`BUSINESS_ID`);

--
-- Indexes for table `business`
--
ALTER TABLE `business`
  ADD PRIMARY KEY (`BUSINESS_ID`);

--
-- Indexes for table `categories`
--
ALTER TABLE `categories`
  ADD PRIMARY KEY (`CATEGORY_ID`);

--
-- Indexes for table `contact`
--
ALTER TABLE `contact`
  ADD PRIMARY KEY (`CONTACT_ID`),
  ADD KEY `FK_CONTACT_CUSTOMER` (`CUSTOMER_ID`);

--
-- Indexes for table `country`
--
ALTER TABLE `country`
  ADD PRIMARY KEY (`COUNTRY_ID`);

--
-- Indexes for table `customers`
--
ALTER TABLE `customers`
  ADD PRIMARY KEY (`CUSTOMER_ID`);

--
-- Indexes for table `employees`
--
ALTER TABLE `employees`
  ADD PRIMARY KEY (`EMPLOYEE_ID`),
  ADD KEY `FK_EMPLOYEE_POSITION` (`POSITION_ID`),
  ADD KEY `FK_PLOYEE_BUSINESS` (`BUSINESS_ID`);

--
-- Indexes for table `menu`
--
ALTER TABLE `menu`
  ADD PRIMARY KEY (`ID`);

--
-- Indexes for table `orders`
--
ALTER TABLE `orders`
  ADD PRIMARY KEY (`ORDER_ID`),
  ADD KEY `FK_ORDER_CUSTOMER` (`CUSTOMER_ID`),
  ADD KEY `FK_ORDER_EMPLOYEE` (`EMPLOYEE_ID`);

--
-- Indexes for table `order_details`
--
ALTER TABLE `order_details`
  ADD PRIMARY KEY (`ORDER_DETAIL_ID`,`ORDER_ID`);

--
-- Indexes for table `position`
--
ALTER TABLE `position`
  ADD PRIMARY KEY (`POSITION_ID`);

--
-- Indexes for table `products`
--
ALTER TABLE `products`
  ADD PRIMARY KEY (`PRODUCT_ID`),
  ADD KEY `FK_PRODUCT_CATEGORY` (`CATEGORY_ID`);

--
-- Indexes for table `product_image`
--
ALTER TABLE `product_image`
  ADD PRIMARY KEY (`ID`);

--
-- Indexes for table `product_supplier`
--
ALTER TABLE `product_supplier`
  ADD PRIMARY KEY (`PRODUCT_ID`),
  ADD KEY `PK_PRODUCT_SUPPLIER_SUPPLIER` (`SUPPLIER_ID`);

--
-- Indexes for table `rol`
--
ALTER TABLE `rol`
  ADD PRIMARY KEY (`ROL_ID`);

--
-- Indexes for table `service`
--
ALTER TABLE `service`
  ADD PRIMARY KEY (`service_id`(1));

--
-- Indexes for table `suppliers`
--
ALTER TABLE `suppliers`
  ADD PRIMARY KEY (`SUPPLIER_ID`);

--
-- Indexes for table `turns`
--
ALTER TABLE `turns`
  ADD PRIMARY KEY (`TURN_ID`);

--
-- Indexes for table `user`
--
ALTER TABLE `user`
  ADD PRIMARY KEY (`USER_ID`),
  ADD KEY `FK_USER_EMPLOYEE` (`EMPLOYEE_ID`),
  ADD KEY `FK_USER_ROL` (`ROL_ID`);

--
-- Constraints for dumped tables
--

--
-- Constraints for table `contact`
--
ALTER TABLE `contact`
  ADD CONSTRAINT `FK_CONTACT_CUSTOMER` FOREIGN KEY (`CUSTOMER_ID`) REFERENCES `customers` (`CUSTOMER_ID`);

--
-- Constraints for table `employees`
--
ALTER TABLE `employees`
  ADD CONSTRAINT `FK_EMPLOYEE_POSITION` FOREIGN KEY (`POSITION_ID`) REFERENCES `position` (`POSITION_ID`),
  ADD CONSTRAINT `FK_PLOYEE_BUSINESS` FOREIGN KEY (`BUSINESS_ID`) REFERENCES `business` (`BUSINESS_ID`);

--
-- Constraints for table `orders`
--
ALTER TABLE `orders`
  ADD CONSTRAINT `FK_ORDER_CUSTOMER` FOREIGN KEY (`CUSTOMER_ID`) REFERENCES `customers` (`CUSTOMER_ID`),
  ADD CONSTRAINT `FK_ORDER_EMPLOYEE` FOREIGN KEY (`EMPLOYEE_ID`) REFERENCES `employees` (`EMPLOYEE_ID`);

--
-- Constraints for table `products`
--
ALTER TABLE `products`
  ADD CONSTRAINT `FK_PRODUCT_CATEGORY` FOREIGN KEY (`CATEGORY_ID`) REFERENCES `categories` (`CATEGORY_ID`);

--
-- Constraints for table `product_supplier`
--
ALTER TABLE `product_supplier`
  ADD CONSTRAINT `PK_PRODUCT_SUPPLIER_PRODUCT` FOREIGN KEY (`PRODUCT_ID`) REFERENCES `products` (`PRODUCT_ID`),
  ADD CONSTRAINT `PK_PRODUCT_SUPPLIER_SUPPLIER` FOREIGN KEY (`SUPPLIER_ID`) REFERENCES `suppliers` (`SUPPLIER_ID`);

--
-- Constraints for table `user`
--
ALTER TABLE `user`
  ADD CONSTRAINT `FK_USER_EMPLOYEE` FOREIGN KEY (`EMPLOYEE_ID`) REFERENCES `employees` (`EMPLOYEE_ID`),
  ADD CONSTRAINT `FK_USER_ROL` FOREIGN KEY (`ROL_ID`) REFERENCES `rol` (`ROL_ID`);

DELIMITER $$
--
-- Events
--
CREATE DEFINER=`comesa20_pruiz`@`%.%.%.%` EVENT `act_cxp_vencido` ON SCHEDULE EVERY 1 DAY STARTS '2023-04-03 03:30:00' ON COMPLETION NOT PRESERVE ENABLE DO UPDATE CUENTAS_POR_PAGAR
	SET ESTADO_DEL_PAGO = 'VE', USUARIO_ACTUALIZO = 'job_system', FECHA_ACTUALIZO = sysdate()
	WHERE ESTADO_DEL_PAGO NOT IN ('VE', 'PA')
    AND ESTADO='A'$$

DELIMITER ;
COMMIT;

/*!40101 SET CHARACTER_SET_CLIENT=@OLD_CHARACTER_SET_CLIENT */;
/*!40101 SET CHARACTER_SET_RESULTS=@OLD_CHARACTER_SET_RESULTS */;
/*!40101 SET COLLATION_CONNECTION=@OLD_COLLATION_CONNECTION */;
